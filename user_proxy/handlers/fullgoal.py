...
@route(r'/fullgoal/oauth-login')
class FULLGOALOauthLoginHandler(BaseHandler):
    """oauth2授权码模式"""
    ...
    def build_user_info(res_data):
        return {
            'uid': res_data['id'],
            'ext_uname': res_data['id'],
            'username': res_data['id'],
        }
    ...
    def get(self, *args, **kwargs):
        ...
        origin_url = f'{trident_base}/api/v1/fullgoal/oauth-login'
        ...
        if access_token:
            ...
            access_token_session_expired = config.get_config('oauth2_auth.access_token_session_expire', 2592000)
            self.session.driver.client.setex(access_token, access_token_session_expired, self.session.driver.db_key(self.session.session_id))
            ...
            try:
                ...
                redirect_url = create_url(trident_base, 'api/v1/fullgoal/oauth-login', *url_args)
...
@route(r'/fullgoal/oauth-logout')
class FULLGOALOauthLogoutHandler(BaseHandler):
    def post(self, *args, **kwargs):
        logging.debug('headers: %s', self.request.headers)
        access_token = self.request.headers.get('AT')
        if access_token:
            logging.info('delete access_token session: %s', access_token)
            session_id = self.session.driver.client.get(access_token)
            if session_id:
                self.session.driver.client.delete(session_id)
            self.session.driver.client.delete(access_token)
        return self.data({})

...
class DubboUtil(object):
    ...
    def conn(self, host, port):
        ...
        try:
            ...
            raise DubboException(f'{host}:{port}, Telnet连接出错: {str(e)}') from e

    def command(self, str_=""):
        # 模拟cmd控制台 dubbo>invoke ...
        self.conn.write(str_.encode(encoding="gb18030") + b'\n')
    ...
    def invoke_command(self, service, method, param):
        ...
        try:
            ...
            # 字节数据解码 gbk
            data = data.decode("gb18030").split('\n')[0].strip()
        self.conn.close()
        return data
    ...
    def ls_command(self, service, method=None):
        ...
        if data.decode("utf-8").startswith('No such service'):
            raise DubboException(f"{service}没有在Zookeeper中心注册")
        data = data.decode("utf-8").split('\n')[:-1]
        method_data = {}
        for dto in data:
        ...
        if method:
            if method in method_data.keys():
                ...
                raise DubboException(f"{service}服务下不存在{method}方法")
        self.conn.close()
        return method_data
    ...
    def invoke(self, service, method, param_dict, param_type_list = None):
        ...
        if param_type_list:
            ...
            if len(error_type_list) > 0:
                ...
                error_msg += '！请检查！'
                raise DubboException(error_msg)
        else:
        ...
        try:
            ...
            raise Exception(f"解析json失败: {response_data}！请检查data请求参数！") from e
...
class ZookeeperUtil(object):
    ...
    def zk_conn(self):
        ...
        try:
            ...
            raise DubboException(f"Zookeeper中心连接失败, 请重试！{str(e)}")
    ...
    def get_service_info(self, service):
        ...
        if not node:
            raise DubboException(f"{service}没有在Zookeeper中心注册")
        if len(node) > 1:
            raise DubboException(f"{service}在Zookeeper中心注册了两次以上！请联系开发处理！")
        # 查询dubbo服务的详细信息
        node_data = self.zk.get_children(f'/dubbo/{node[0]}/providers')
        if not node_data:
            raise DubboException(f'{node[0]}服务没有提供者！')
...
class DubboException(Exception):
    def __init__(self, msg, error_type=None, data=None, status_code=400):
        self.msg = msg
        self.status_code = status_code
        self.data = data
        self.error_type = error_type
        super().__init__(self, msg, data)

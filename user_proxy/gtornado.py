...
class TornadoWorker(Worker):
    ...
    def run(self):
        ...
        app_params = {"max_buffer_size": 1024**3, "xheaders": True}
        ...
        if self.cfg.is_ssl:
            ...
            if TORNADO5:
                server = server_class(app, ssl_options=_ssl_opt, **app_params)
            else:
                server = server_class(app, io_loop=self.ioloop, ssl_options=_ssl_opt, **app_params)
        else:
            if TORNADO5:
                server = server_class(app, **app_params)
            else:
                server = server_class(app, io_loop=self.ioloop, **app_params)
